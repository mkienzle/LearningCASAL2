...
plot(x, y0, xlab = "", ylab = "Abundance (nb)", las = 1, xlim = c(1985, 2010), ylim = c(min(Y3), max(y0)), pch = pchar)
points(x + 0.5, y1, pch = pchar)
points(x + 0.5, y2, pch = pchar)
points(x + 1, y3, pch = pchar)
for(i in 1:length(x)){
      #print(x[i])
      curve(y0[i] * exp(-sim$preExploitationPop$M * (t-x[i])), from = x[i], to = x[i]+1, xname = "t", add = TRUE, col = "green", lwd = 2.0)
      }
...
for(i in 1:length(X)){
      curve(Y0[i] * exp(-sim$DuringExploitationPop$M * (t-X[i])), from = X[i], to = X[i]+0.5, xname = "t", add = TRUE, col = "green", lwd = 2.0)
      curve(Y2[i] * exp(-sim$DuringExploitationPop$M * (t-X[i]-0.5)), from = X[i] + 0.5, to = X[i]+1, xname = "t", add = TRUE, col = "green", lwd = 2.0)
      }
